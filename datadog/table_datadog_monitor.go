...
func tableDatadogMonitor(ctx context.Context) *plugin.Table {
	return &plugin.Table{
		...
		Columns: []*plugin.Column{
			...
			{Name: "state_groups", Type: proto.ColumnType_JSON, Transform: transform.FromField("State.Groups"), Description: "Dictionary where the keys are groups (comma separated lists of tags) and the values are the list of groups your monitor is broken down on."},
			{Name: "tags", Type: proto.ColumnType_STRING, Description: "Tags associated to monitor."},
		},
	}
}
